...
class CacheManager:
    ...
    def _cleanup_memory_cache(self):
        ...
        sorted_keys = sorted(
            self.memory_access_times.keys(), key=lambda k: self.memory_access_times[k]
        )
...
def get_cache_manager(
    redis_url: Optional[str] = None, default_ttl: int = 3600, max_memory_size: int = 1000
) -> CacheManager:
    """
    ...
    if _cache_manager is None:
        _cache_manager = CacheManager(
            redis_url=redis_url, default_ttl=default_ttl, max_memory_size=max_memory_size
        )
...
def cached(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            # Generate cache key
            cache_key = manager._make_key(key_prefix, func.__name__, *args, **kwargs)
...
def cache_news_items(category: str, subcategory: str, items: List[Dict], ttl: int = 1800):
    ...
    cache_manager.set(
        cache_key, items, ttl=ttl, tags=[f"news:{category}", f"news:{category}:{subcategory}"]
    )
    return len(items)
