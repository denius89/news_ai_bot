...
FEW_SHOT_EXAMPLES = [
    {
        ...
        "output": {
            ...
            "summary": "Bitcoin торгуется на уровне $50,200, согласно данным CoinDesk. Ethereum достиг $3,200 (источник: CoinMarketCap). Рост произошел после заявления SEC о рассмотрении нового Bitcoin ETF.",
            "why_important": [
                "Bitcoin превысил психологически важный уровень $50,000",
                "Ethereum обновил исторический максимум",
                "SEC рассматривает новые ETF на Bitcoin",
            ],
            "context": "Рост произошел после публикации документа SEC о Bitcoin ETF (Reuters, 15 января).",
            "what_next": "Решение по Bitcoin ETF ожидается в течение недели.",
    ...
    },
    {
        ...
        "output": {
            ...
            "summary": "OpenAI официально запустила GPT-5 с заявленными улучшениями в рассуждениях (источник: OpenAI blog, 20 января). Google анонсировал Gemini Pro с интеграцией в Workspace (источник: Google Cloud blog).",
            "why_important": [
                "GPT-5 обещает улучшения в логическом мышлении на 40%",
                "Gemini Pro интегрирован в Google Docs и Gmail",
                "Обе модели доступны с 25 января 2024",
            ],
            "context": "Анонсы последовали после исследования Stanford HAI о прогрессе в ИИ за 2023 год.",
            "what_next": "Техническая документация моделей будет опубликована 30 января.",
...
ЗАДАЧА: Создать точный и информативный дайджест на основе предоставленных новостей.

🚨 КРИТИЧЕСКИ ВАЖНО — ПОВЕРКА ФАКТОВ:
- ИСПОЛЬЗУЙ ТОЛЬКО факты из предоставленных новостей
- НЕ добавляй информацию, которой нет в источниках
- НЕ делай предположений и не домысливай детали
- НЕ выдумывай цифры, даты или цитаты
- Если информации недостаточно — признай это честно

СТИЛЬ НАПИСАНИЯ:
✓ Краткость и точность — только суть
✓ Конкретные факты с источниками
✓ Логичная структура — от главного к деталям
✓ Минимум "воды" — каждое предложение информативно
✓ Используй реальные цитаты и данные из новостей

СТРУКТУРА:
1. Главная новость (1-2 предложения)
2. Ключевые факты и цифры
3. Влияние на рынок/отрасль (только на основе фактов)

ЗАПРЕЩЕНО:
❌ Добавлять информацию не из источников
❌ Использовать общие фразы без конкретики
❌ Писать "возможно", "вероятно", "эксперты считают" без ссылок
❌ Пропускать проверку фактов
❌ Создавать "захватывающие" истории из ничего

ОБЯЗАТЕЛЬНО:
✅ Указывать источники для каждого факта
✅ Использовать точные цифры из новостей
✅ Ссылаться на конкретные события и даты
...
Помни: лучше короткий точный дайджест, чем длинный с выдумками.
...
- НЕ добавляй информацию, которой нет в источниках
- Если информации мало — сделай короче, но точно
- Указывай источники для каждого факта в summary
- Избегай общих фраз и "воды"

ПРОВЕРЬ ПЕРЕД ОТПРАВКОЙ:
✅ Каждый факт есть в источниках?
✅ Указаны источники?
✅ Нет выдуманной информации?
✅ Текст информативен и не содержит "воды"?
