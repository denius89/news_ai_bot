...
└── tools/
    ...
    └── show_news.py
```
> ℹ️ **Примечание по структуре проекта**  
> Файл `CODEMAP.md` генерируется автоматически скриптом [`tools/repo_map.py`](tools/repo_map.py).  
> Если структура в `MASTER_FILE.md` и `CODEMAP.md` различаются — приоритет имеет `CODEMAP.md`.  
> Обновить карту проекта можно командой:
> ```bash
> python tools/repo_map.py
> ```
