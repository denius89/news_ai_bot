...
Запускает процесс загрузки и сохранения новостей.
...
### Утилита `tools/fix_old_news.py` (план)
Предназначена для обновления старых записей в базе:
- Добавление `source`, если он отсутствует.  
- Обновление пустого `content`, если появилось описание.  
- Возможность выборочного исправления через флаги.  

> ⚠️ Утилита пока в планах, в MVP может быть реализована позже.
