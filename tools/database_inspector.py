...
class DatabaseInspector:
    ...
    def check_users_table(self) -> Dict:
        ...
        try:
            ...
            columns = list(result.data[0].keys())
            count_result = self.service.sync_client.table("users").select("id", count="exact").execute()
    ...
    def check_user_notifications(self) -> Dict:
        ...
        try:
            result = self.service.sync_client.table("user_notifications").select("*").limit(5).execute()
            ...
            columns = list(result.data[0].keys())
            count_result = self.service.sync_client.table("user_notifications").select("id", count="exact").execute()
            total_count = count_result.count if hasattr(count_result, 'count') else len(result.data)
            ...
            logger.info(f"✅ User notifications table: {total_count} notifications, {len(columns)} columns")
            logger.debug(f"   Columns: {', '.join(columns)}")
    ...
    def check_subcategory_migration(self) -> Dict:
        ...
        # Check news table
        try:
            news_result = self.service.sync_client.table("news").select("subcategory").limit(1).execute()
            if news_result.data:
        ...
        # Check events table
        try:
            events_result = self.service.sync_client.table("events").select("subcategory").limit(1).execute()
    ...
    def check_database_health(self) -> Dict:
        ...
        logger.info(f"🎯 Database health: {health_report['summary']['healthy_tables']}/{health_report['summary']['total_tables']} tables healthy")
    ...
    def generate_report(self, checks: List[str]) -> str:
        ...
        for check in checks:
            if check == "all-columns":
                ...
                report_lines.append(f"Overall: {'✅ Healthy' if results['summary']['healthy'] else '❌ Issues Found'}")
                report_lines.append(f"Tables: {results['summary']['healthy_tables']}/{results['summary']['total_tables']}")
                if results['issues']:
...
def main():
    ...
    parser.add_argument("check", nargs="+", choices=[
        "all-columns", "users", "notifications", "user-notifications", 
    ...
    if "all" in args.check:
        args.check = ["all-columns", "users", "notifications", "user-notifications", "subcategory", "health"]
