...
def optimize_database():
    ...
    indexes = [
        # News indexes
        ("news", "idx_news_published_at", "CREATE INDEX IF NOT EXISTS idx_news_published_at ON news (published_at DESC)"),
        ("news", "idx_news_source", "CREATE INDEX IF NOT EXISTS idx_news_source ON news (source)"),
        ("news", "idx_news_category", "CREATE INDEX IF NOT EXISTS idx_news_category ON news (category)"),
        ("news", "idx_news_credibility", "CREATE INDEX IF NOT EXISTS idx_news_credibility ON news (credibility DESC)"),
        
        # Events indexes
        ("events", "idx_events_time_desc", "CREATE INDEX IF NOT EXISTS idx_events_time_desc ON events (event_time DESC)"),
        ("events", "idx_events_country", "CREATE INDEX IF NOT EXISTS idx_events_country ON events (country)"),
        ("events", "idx_events_importance", "CREATE INDEX IF NOT EXISTS idx_events_importance ON events (importance)"),
        
        # Users indexes
        ("users", "idx_users_telegram", "CREATE INDEX IF NOT EXISTS idx_users_telegram ON users (telegram_id)"),
        
        # User notifications indexes
        ("user_notifications", "idx_notifications_user", "CREATE INDEX IF NOT EXISTS idx_notifications_user ON user_notifications (user_id)"),
        ("user_notifications", "idx_notifications_read", "CREATE INDEX IF NOT EXISTS idx_notifications_read ON user_notifications (read)"),
        ("user_notifications", "idx_notifications_timestamp", "CREATE INDEX IF NOT EXISTS idx_notifications_timestamp ON user_notifications (timestamp DESC)"),
    ]
    ...
    try:
        ...
        if result.data:
            ...
            users_result = supabase.table('users').select('id').in_('id', list(unique_user_ids)).execute()
            ...
            if orphaned_notifications:
                print(f"   ⚠️  Найдены уведомления с несуществующими user_id: {len(orphaned_notifications)}")
            else:
                print(f"   ✅ Все уведомления имеют корректные ссылки на пользователей")
    except Exception as e:
