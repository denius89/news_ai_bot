...
class SourceChecker:
    ...
    def check_all_sources(self):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_task = {
                executor.submit(self.check_single_source, *task): task 
                for task in tasks
            }
            ...
            for future in as_completed(future_to_task):
                try:
                    ...
                    if completed % 10 == 0 or completed == total_sources:
                        print(f"   📊 Проверено: {completed}/{total_sources} ({completed/total_sources*100:.1f}%)")
    ...
    def generate_csv_report(self, output_file="logs/sources_check.csv"):
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['category', 'subcategory', 'name', 'url', 'status', 'rss_ok', 'http_status', 'final_url', 'elapsed_ms', 'error']
        ...
        with open(output_file, 'w', encoding='utf-8') as md_file:
            ...
            if error_sources:
                ...
                for source in error_sources[:20]:  # Показываем топ 20
                    md_file.write(f"| {source['category']} | {source['subcategory']} | {source['name']} | {source['status']} | {source['url']} |\n")
            ...
            for cat, stats in category_stats.items():
                ...
                md_file.write(f"| {cat} | {stats['total']} | {stats['ok']} | {stats['error']} | {success_rate:.1f}% |\n")
