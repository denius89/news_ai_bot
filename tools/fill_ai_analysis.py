...
def get_news_without_ai_analysis():
    ...
    try:
        ...
        if result.data:
            logger.info(
                f"📊 Найдено {len(result.data)} новостей с дефолтными или отсутствующими AI оценками"
            )
            return result.data
        else:
...
def update_news_ai_analysis(news_uid, credibility, importance):
    ...
    try:
        result = safe_execute(
            supabase.table("news")
            .update({"credibility": credibility, "importance": importance})
            .eq("uid", news_uid)
        )
    ...
    for news_item in news_list:
        try:
            ...
            if update_news_ai_analysis(news_uid, credibility, importance):
                processed += 1
                logger.info(
                    f"✅ Обновлено: credibility={credibility:.2f}, importance={importance:.2f}"
                )
            else:
                errors += 1
