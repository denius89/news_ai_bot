...
class SourceChecker:
    ...
    async def __aenter__(self):
        self.session = aiohttp.ClientSession(
            ...
            headers={'User-Agent': 'Mozilla/5.0 (compatible; PulseAI Bot)'}
        )
        return self
    ...
    def print_statistics(self) -> None:
        ...
        print(f"📊 Процент валидности: {(self.valid_sources/self.total_sources*100):.1f}%" if self.total_sources > 0 else "0%")
        
        # Топ 5 невалидных источников
        invalid_results = [(cat, sub, name, url, result) for cat, sub, name, url, result in self.results if "❌" in result]
