    ...
    try:
        ...
        try:
            users_result = db.safe_execute(
                db.sync_client.table('users').select('id', count='exact')
            )
        ...
        try:
            ...
            if ai_result.data:
                importance_values = [float(item['importance']) for item in ai_result.data if item.get('importance') is not None]
                credibility_values = [float(item['credibility']) for item in ai_result.data if item.get('credibility') is not None]
...
def get_ai_metrics():
    ...
    try:
        ...
        return jsonify({
            ...
            'avg_importance': round(sum(importance_values) / len(importance_values), 2) if importance_values else 0.0,
            'avg_credibility': round(sum(credibility_values) / len(credibility_values), 2) if credibility_values else 0.0,
...
def get_user_metrics():
    ...
    try:
        ...
        subscriptions_result = db.safe_execute(
            db.sync_client.table('subscriptions')
...
def get_logs():
    ...
    try:
        ...
        for line in last_lines:
            parsed_logs.append({
                'text': line.strip(),
                'timestamp': datetime.now().isoformat()  # TODO: парсить реальный timestamp
            })
...
def get_config():
    ...
    try:
        from config.core.settings import (
            AI_MODEL_SUMMARY, AI_MODEL_SCORING, AI_MAX_TOKENS,
            REACTOR_ENABLED, DEBUG, APP_ENV
        )
        ...
        return jsonify({
            ...
            'api_keys': {
                'openai': '****' + os.getenv('OPENAI_API_KEY', '')[-4:] if os.getenv('OPENAI_API_KEY') else 'not set',
                'telegram_bot': '****' + os.getenv('TELEGRAM_BOT_TOKEN', '')[-4:] if os.getenv('TELEGRAM_BOT_TOKEN') else 'not set'
            }
        })
...
def get_prompts():
    ...
    try:
        ...
        return jsonify({
            'styles': STYLE_CARDS,
            'tones': TONE_CARDS,
            'editable': False  # v1: только просмотр
        })
        ...
        by_category = [
            {
                'category': cat,
                'count': stats['count'],
                'avg_importance': round(sum(stats['importance']) / len(stats['importance']), 2) if stats['importance'] else 0,
                'avg_credibility': round(sum(stats['credibility']) / len(stats['credibility']), 2) if stats['credibility'] else 0
            }
...
def get_user_engagement_metrics():
    ...
    try:
        ...
        users_month = db.safe_execute(
            db.sync_client.table('users')
            .select('created_at')
        ...
        growth_timeline = [
            {'date': date, 'count': count}
            for date, count in sorted(growth_dict.items())
        ]
        
        # Subscriptions distribution
        subs_result = db.safe_execute(
            db.sync_client.table('subscriptions')
            .select('category')
            .limit(5000)
        )
        ...
        timeline = [
            {
                ...
                'calls': count,
                'tokens': count * 500,
                'cost': round((count * 500 / 1_000_000) * 0.15, 4)
            }
            for date, count in sorted(timeline_dict.items())
        ]
...
def get_system_health():
    ...
    try:
        ...
        return jsonify({
            ...
            'uptime': {
                'flask_uptime_seconds': flask_status['uptime_seconds']
            }
        })
