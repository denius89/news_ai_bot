...
class TestRejectionAnalyzer:
    ...
    def test_analyze_rejections_insufficient_samples(self):
        ...
        with open(self.temp_log.name, "w") as f:
            f.write(
                '[2025-01-01T10:00:00Z] REJECTED: reason=pre_filter category=crypto source=example.com title="Bitcoin"\n'
            )
            f.write(
                '[2025-01-01T11:00:00Z] REJECTED: reason=pre_filter category=tech source=tech.com title="Tech news"\n'
            )
    ...
    def test_analyze_rejections_with_data(self):
        ...
        with open(self.temp_log.name, "w") as f:
            for i in range(10):
                f.write(
                    f'[2025-01-01T{i:02d}:00:00Z] REJECTED: reason=pre_filter category=crypto source=spam.com title="Bitcoin giveaway click here"\n'
                )
    ...
    def test_generate_recommendations(self):
        ...
        with open(self.temp_log.name, "w") as f:
            for i in range(20):
                f.write(
                    f'[2025-01-01T{i:02d}:00:00Z] REJECTED: reason=pre_filter category=crypto source=spam.com title="Bitcoin giveaway scam"\n'
                )
...
class TestAutoRuleManager:
    ...
    def test_load_and_save_rules(self):
        ...
        test_rules = {"stop_markers": ["test", "example"], "auto_generated": {"stop_markers": []}}
    ...
    def test_add_stop_markers(self):
        ...
        rules = {"stop_markers": ["existing"], "auto_generated": {"stop_markers": []}}
    ...
    def test_cleanup_old_rules(self):
        ...
        rules = {
            "auto_generated": {
                "stop_markers": [
                    {"word": "old_word", "created_at": old_date.isoformat(), "confidence": 0.5},
                    {"word": "recent_word", "created_at": recent_date.isoformat(), "confidence": 0.6},
                    {
                        "word": "high_conf_old",
    ...
    def test_get_active_auto_rules_count(self):
        ...
        rules = {
            "auto_generated": {
                "stop_markers": [{"word": "word1"}, {"word": "word2"}],
                "source_blacklist": [{"source": "spam.com"}],
            }
        }
...
class TestAutoLearningIntegration:
    ...
    def setup_method(self):
        ...
        config = {
            ...
            "rejection_analysis": {"top_words_limit": 10, "frequency_threshold": 0.2},
        }
        ...
        with open(self.temp_log.name, "w") as f:
            for i in range(10):
                f.write(
                    f'[2025-01-01T{i:02d}:00:00Z] REJECTED: reason=pre_filter category=crypto source=spam.com title="Bitcoin giveaway scam click here"\n'
                )
