...
class ContentScheduler:
    ...
    def _initialize_time_windows(self) -> List[TimeWindow]:
        ...
        for window_name, window_config in schedule.items():
            ...
            logger.info(
                f"Initialized time window: {window_name} ({window.start_hour}:00-{window.end_hour}:00) -> {window.categories}"
            )
    ...
    def filter_digests_by_schedule(self, digests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Filtered digests: {len(filtered_digests)} suitable, {skipped_count} skipped for window '{current_window.name}'"
        )
