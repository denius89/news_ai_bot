...
from database.service import upsert_news, async_upsert_news, get_sync_service, get_async_service  # noqa: E402
from utils.error_handler import handle_network_error, handle_parsing_error, NetworkError, ParsingError  # noqa: E402
...
class UnifiedParserService:
    ...
    def parse_source(
        ...
        if self.async_mode:
            # Run async version in sync context
            return asyncio.run(self.async_parse_source(url, category, subcategory, source_name, limit))
    ...
    def _parse_source_sync(
        ...
        try:
            ...
            raise ParsingError(f"Failed to parse {source_name}", source=source_name, url=url, cause=e)
...
def parse_source(url: str, category: str, subcategory: str, source_name: str) -> List[Dict[str, Any]]:
    """Backward compatibility function for parse_source."""
    service = get_sync_parser_service()
    return service.parse_source(url, category, subcategory, source_name)


async def async_parse_source(url: str, category: str, subcategory: str, source_name: str) -> List[Dict[str, Any]]:
