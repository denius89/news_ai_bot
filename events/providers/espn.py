...
class ESPNProvider:
    ...
    def _get_daily_events(self, date) -> List[Event]:
        ...
        if date.weekday() >= 5:  # Saturday or Sunday
            ...
            if date.weekday() == 6:  # Sunday
                events.append(Event(
                    ...
                    starts_at=datetime.combine(date, datetime.min.time().replace(hour=17, minute=0)).replace(tzinfo=timezone.utc),
                    ends_at=datetime.combine(date, datetime.min.time().replace(hour=23, minute=0)).replace(tzinfo=timezone.utc),
                    source="espn.com",
            ...
            events.append(Event(
                ...
                category="sports",
                subcategory="soccer",
                starts_at=datetime.combine(date, datetime.min.time().replace(hour=14, minute=30)).replace(tzinfo=timezone.utc),
                ends_at=datetime.combine(date, datetime.min.time().replace(hour=17, minute=0)).replace(tzinfo=timezone.utc),
                source="espn.com",
        ...
        # Weekday events
            ...
            if date.weekday() in [1, 3]:  # Tuesday, Thursday
                events.append(Event(
                    ...
                    starts_at=datetime.combine(date, datetime.min.time().replace(hour=19, minute=0)).replace(tzinfo=timezone.utc),
                    ends_at=datetime.combine(date, datetime.min.time().replace(hour=22, minute=0)).replace(tzinfo=timezone.utc),
                    source="espn.com",
            ...
            if date.day % 7 == 1:  # First week of month
                events.append(Event(
                    ...
                    category="sports",
                    subcategory="tennis",
                    starts_at=datetime.combine(date, datetime.min.time().replace(hour=10, minute=0)).replace(tzinfo=timezone.utc),
                    ends_at=datetime.combine(date, datetime.min.time().replace(hour=18, minute=0)).replace(tzinfo=timezone.utc),
                    source="espn.com",
        ...
        if date.day == 15:  # Mid-month
            events.append(Event(
                ...
                category="sports",
                subcategory="general",
                starts_at=datetime.combine(date, datetime.min.time().replace(hour=20, minute=0)).replace(tzinfo=timezone.utc),
                ends_at=datetime.combine(date, datetime.min.time().replace(hour=23, minute=0)).replace(tzinfo=timezone.utc),
                source="espn.com",
    ...
    def _calculate_importance(self, title: str) -> float:
        ...
        if any(keyword in title_lower for keyword in ['championship', 'final', 'playoff', 'world series', 'stanley cup']):
            return 0.9
