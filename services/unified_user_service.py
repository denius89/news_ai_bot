...
class UnifiedUserService:
    ...
    def get_or_create_user(
        self, telegram_id: int, username: Optional[str] = None, locale: str = "ru"
    ) -> str:
        """
    ...
    async def async_get_or_create_user(
        ...
        try:
            ...
            # Create new user if not found
            user_id = await self.async_service.upsert_user_by_telegram(
                telegram_id, username, locale
            )
            logger.info("Created new user: ID=%s, telegram_id=%s", user_id, telegram_id)
            return user_id
    ...
    def list_notifications(self, user_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.debug(
                "Retrieved %d notifications for user_id=%s", len(user_notifications), user_id
            )
    ...
    async def async_list_notifications(self, user_id: int) -> List[Dict]:
        ...
        try:
            ...
            user_notifications = [n for n in notifications if n.get("user_id") == user_id]
            logger.debug(
                "Retrieved %d async notifications for user_id=%s", len(user_notifications), user_id
            )
            return user_notifications
