...
class NotificationDeliveryService:
    ...
    async def send_notification(
        ...
        try:
            ...
            if via_telegram:
                ...
                if telegram_success:
                    logger.info(f"✅ Уведомление {notification_id} отправлено через Telegram для user_id={user_id}")
                else:
                    logger.warning(f"⚠️ Не удалось отправить уведомление {notification_id} через Telegram для user_id={user_id}")
            
            logger.info(f"✅ Уведомление {notification_id} создано и доставлено для user_id={user_id}")
    ...
    async def send_bulk_notification(
        ...
        logger.info(f"📊 Массовая отправка завершена: успешно={results['success']}, ошибок={results['failed']}")
        return results
