...
class TelegramNotificationService:
    ...
    async def send_notification_via_bot(
        ...
        try:
            ...
            logger.info(f"✅ Notification sent to user {user_id}, notification_id: {notification_id}")
            return True
    ...
    def _create_mark_read_keyboard(self, notification_id: str) -> types.InlineKeyboardMarkup:
        ...
        builder.button(
            ...
            callback_data=f"mark_read:{notification_id}"
        )
    ...
    async def handle_mark_read_callback(
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"✅ Notification {notification_id} marked as read by user {query.from_user.id}")
                return True
            else:
    ...
    async def _mark_notification_read_via_api(self, user_id: int, notification_id: str) -> bool:
        ...
        try:
            ...
            if result:
                logger.info(f"✅ API: Notification {notification_id} marked as read for user {user_id}")
                return True
            else:
                logger.warning(f"⚠️ API: Failed to mark notification {notification_id} as read for user {user_id}")
                return False
