...
import { BarChart3, Bot, Calendar, Newspaper, Rocket, Settings, Sparkles } from 'lucide-react';
...
import { logDevicePerformanceInfo, shouldReduceMotion } from '../utils/performance';
...
const HomePage: React.FC<HomePageProps> = ({ theme, onThemeToggle, onNavigate }) => {
    ...
    const quickActions = [
        ...
        {
            title: 'AI Дайджест',
            description: 'AI-дайджесты — только важное',
            page: 'digest',
            icon: <Bot className="w-6 h-6 text-primary" />,
        },
        {
            title: 'События',
            description: 'Что важно на этой неделе',
            page: 'events',
    ...
    return (
        <div className="min-h-screen bg-bg">
            <Header
                title="PulseAI"
                subtitle="Умные новости каждый день"
                theme={theme}
                        ...
                        <Card className="text-center py-8 bg-ai-holo border-primary/20">
                            <CardContent>
                                ...
                                <p className="text-muted-strong text-lg mb-6 max-w-2xl mx-auto">
                                    {telegramUser ?
                                        "AI уже изучает новости и готовит тебе подборку" :
                                        "PulseAI — новости, которые работают на тебя"
                                    }
                                </p>
                                <Button
                                    size="lg"
                                        ...
                                        <CardContent>
                                            ...
                                            >
                                                {action.title === 'AI Дайджест' ? 'Открыть дайджесты' :
                                                    action.title === 'События' ? 'Открыть события' :
                                                        action.title === 'Настройки' ? 'Настроить' : 'Открыть'}
                                            </Button>
                                        </CardContent>
                                    </Card>
                                </motion.div>
