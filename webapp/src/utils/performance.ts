...
/**
 * Возвращает motion props в зависимости от устройства
 * На мобильных устройствах отключает анимации
 */
export function getMotionProps(props: any): any {
    if (isMobileDevice() || shouldReduceMotion()) {
        return {}; // Полностью отключаем анимации на мобильных
    }
    return props;
}

/**
 * Возвращает компонент для motion элементов
 * На мобильных устройствах использует обычный div
 */
export function getMobileMotionComponent() {
    if (isMobileDevice() || shouldReduceMotion()) {
        return 'div'; // Используем обычный div вместо motion.div
    }
    return 'motion.div';
}
