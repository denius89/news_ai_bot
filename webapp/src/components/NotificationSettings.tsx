...
export const NotificationSettings: React.FC = () => {
    ...
    const savePreferences = async () => {
        try {
            ...
            if (data.success) {
                showMessage('success', 'Сохранили!');
            } else {
                showMessage('error', 'Не удалось сохранить');
            }
        } catch (error) {
        ...
        <main className="pb-32 pt-4 px-4 max-w-md mx-auto">
            ...
            {message && (
                <motion.div
                    ...
                    exit={{ opacity: 0, y: -20 }}
                    className={`mb-4 p-3 rounded-xl flex items-center gap-2 ${message.type === 'success'
                ...
                <div className="space-y-2">
                    {CATEGORIES.map(category => (
                        <motion.button
                            ...
                            className={`w-full p-3 rounded-xl flex items-center justify-between transition-all ${preferences.categories.includes(category.id)
                ...
                <div className="space-y-2">
                    {FREQUENCIES.map(freq => (
                        <motion.button
                            ...
                            className={`w-full p-3 rounded-xl flex items-center justify-between transition-all ${preferences.notification_frequency === freq.id
                                    ? 'bg-emerald-50 dark:bg-emerald-900/20 border-2 border-emerald-500'
                                    : 'bg-gray-50 dark:bg-gray-800 border-2 border-transparent'
                                }`}
                        >
                ...
                <div className="space-y-2">
                    {DELIVERY_METHODS.map(method => (
                        <motion.button
                            ...
                            className={`w-full p-3 rounded-xl flex items-center justify-between transition-all ${preferences.delivery_method === method.id
                                    ? 'bg-emerald-50 dark:bg-emerald-900/20 border-2 border-emerald-500'
                                    : 'bg-gray-50 dark:bg-gray-800 border-2 border-transparent'
                                }`}
                        >
            ...
            <motion.button
                ...
                className={`w-full py-4 rounded-xl font-semibold text-white transition-all ${saving || preferences.categories.length === 0
