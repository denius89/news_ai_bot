...
class ServiceMonitor:
    ...
    def __init__(self):
        self.services = {
            "flask": {"name": "Flask WebApp", "port": 8001, "path": "/webapp", "pid_file": ".flask.pid"},
            "bot": {"name": "Telegram Bot", "pid_file": ".bot.pid"},
        }

    def check_flask(self) -> Dict:
        """Проверяет состояние Flask."""
        service = self.services["flask"]
        result = {"name": service["name"], "status": "unknown", "details": {}}

        try:
            # Проверяем HTTP доступность
            response = requests.get(f"http://localhost:{service['port']}{service['path']}", timeout=5)
    ...
    def check_all_services(self) -> List[Dict]:
        ...
        return [self.check_flask(), self.check_bot()]
...
def main():
    ...
    parser.add_argument(
        "--interval", "-i", type=int, default=30, help="Интервал проверки в секундах (по умолчанию: 30)"
    )
