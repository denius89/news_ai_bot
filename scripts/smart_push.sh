...
check_code_quality() {
    ...
    # 2. Flake8 проверка стиля (мягкая проверка)
    echo -e "${YELLOW}🔍 Проверка Flake8...${NC}"
    if ! python3 -m flake8 --max-line-length=100 --ignore=E203,W503,E501,F401,F841,F541,W291,W293,E402,E722 .; then
        echo -e "${YELLOW}⚠️ Flake8 нашел проблемы в коде (продолжаем)${NC}"
        echo -e "${BLUE}💡 Основные проблемы: неиспользуемые импорты, переменные, форматирование${NC}"
        echo -e "${BLUE}💡 Для строгой проверки запустите: python3 -m flake8 --max-line-length=100 --ignore=E203,W503 .${NC}"
    else
