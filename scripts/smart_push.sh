...
# Функция для детальной проверки и исправления кода
check_code_quality() {
    echo -e "${BLUE}🧹 Детальная проверка и исправление кода...${NC}"
    
    # Запускаем детальный скрипт исправления
    if [ -f "./scripts/detailed_fix.sh" ]; then
        echo -e "${YELLOW}🔧 Запуск детального исправления ошибок...${NC}"
        ./scripts/detailed_fix.sh
        echo -e "${GREEN}✅ Детальное исправление завершено${NC}"
    else
        echo -e "${YELLOW}⚠️ Детальный скрипт не найден, используем базовую проверку...${NC}"
        
        # Базовая проверка как fallback
